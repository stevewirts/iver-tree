...
const underline = function(config, gc, text, x, y, thickness) {
...
const roundRect = function(gc, x, y, width, height, radius, fill, stroke) {
	if (!stroke) {
		stroke = true;
	}
	if (!radius) {
...
export default class CellProvider {
	...
	getCell(config) {
		const cell = this.cellCache.simpleCellRenderer;
		cell.config = config;
		return cell;
	}

	getTopLeftCell(config) {
		var cell = this.cellCache.emptyCellRenderer;
		cell.config = config;
		return cell;
	}

	getFixedColumnCell(config) {
		var cell = this.cellCache.simpleCellRenderer;
		cell.config = config;
		return cell;
	}

	getFixedRowCell(config) {
		var cell = this.cellCache.simpleCellRenderer;
		cell.config = config;
		return cell;
	}

	defaultCellPaint(gc, x, y, width, height, isLink) {
