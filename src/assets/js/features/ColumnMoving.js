...
let columnAnimationTime = 150;
let dragger;
let draggerCTX;
let floatColumn;
let floatColumnCTX;

export default class ColumnMoving extends FeatureBase {


	constructor() {
		super();
		this.floaterAnimationQueue = [];
	...
	initializeAnimationSupport(grid) {
		noop(grid);
		if (!dragger) {
	...
	floatColumnTo(grid, draggedToTheRight) {
	...
	doColumnMoveAnimation(grid, floaterStartX, draggerStartX) {
		var self = this;
		return function() {
			var d = floatColumn;
	...
	createFloatColumn(grid, columnIndex) {
		var scrollLeft = grid.getHScrollValue();
		var numFixedColumns = grid.getFixedColumnCount();
	...
	setCrossBrowserProperty(element, property, value) {
	...
	createDragColumn(grid, x, columnIndex) {
		var scrollLeft = grid.getHScrollValue();
		var numFixedColumns = grid.getFixedColumnCount();
	...
	_checkAutoScrollToRight(grid, x) {
		if (!this.columnDragAutoScrollingRight) {
			return;
		}
	...
	findNewPositionOnScrollRight(dragIndex) {
		noop(dragIndex);
